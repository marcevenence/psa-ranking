...
export class TournamentService {
  ...
  protected convertDateFromClient(tournament: ITournament): ITournament {
    const copy: ITournament = Object.assign({}, tournament, {
      playDate: tournament.playDate != null && tournament.playDate.isValid() ? tournament.playDate.toJSON() : null,
      endInscriptionDate:
        tournament.endInscriptionDate != null && tournament.endInscriptionDate.isValid() ? tournament.endInscriptionDate.toJSON() : null,
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((tournament: ITournament) => {
        tournament.playDate = tournament.playDate != null ? moment(tournament.playDate) : null;
        tournament.endInscriptionDate = tournament.endInscriptionDate != null ? moment(tournament.endInscriptionDate) : null;
